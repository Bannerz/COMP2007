...
public class HotbarController : MonoBehaviour
{
    ...
    {
        if (slotItems == null || index < 0 || index >= slotItems.Length)
            return null;

        return slotItems[index];
    }

    public bool TrySetSlotItem(int index, GameObject item)
    {
        EnsureSlotItemArray();
    ...
    public bool TryAddSlotItem(GameObject item, out int slotIndex)
    {
        EnsureSlotItemArray();
    ...
    private void EnsureSlotItemArray()
    {
        if (slotItems != null && slotItems.Length == SlotCount)
            return;

        GameObject[] resized = new GameObject[SlotCount];

        if (slotItems != null)
        {
            int copyCount = Mathf.Min(slotItems.Length, resized.Length);
            Array.Copy(slotItems, resized, copyCount);
        }
